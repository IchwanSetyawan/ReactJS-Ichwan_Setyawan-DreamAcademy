...
function formatCustomDate(str) {
  ...
  const newProduct = products.map((item) => {
    const expiredIn = getDiff2(formatCustomDate(item.expiredDate), formatCustomDate(today));
    return {
      productionDate: item.productionDate,
      expiredDate: item.expiredDate,
      shelfLife: getDiff2(formatCustomDate(item.expiredDate), formatCustomDate(item.productionDate)),
      age: getDiff2(formatCustomDate(today), formatCustomDate(item.productionDate)),
      expiredIn,
